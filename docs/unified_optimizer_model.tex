...
\begin{itemize}[noitemsep]
  ...
  \item $\text{triad}_t$: TRIAD penalty (e.g. 100) if slot $t$ is TRIAD, else 0.
...
\begin{equation}
  \min \quad \sum_{t,v} \big( p_t + \sigma_t + \text{triad}_t \big)\, (0.5\, P_{t,v}) + \lambda \sum_v s_v.
\end{equation}

\subsection{Integrated}
Maximize weighted allocation term minus weighted scheduling term:
\begin{equation}
  \max \quad \alpha\, \Bigl( W_{\text{route}} \sum_r y_r + \sum_i c_i\, x_i \Bigr) - \beta\, \Bigl( \sum_{t,v} (p_t + \sigma_t + \text{triad}_t)\, (0.5\, P_{t,v}) + \lambda \sum_v s_v \Bigr).
\end{equation}
...
\begin{tabular}{lll}
  ...
  TRIAD & TRIAD penalty (when active) & 100 \\
